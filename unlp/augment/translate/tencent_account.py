...
def translate_tencent(text, from_='zh', to_='en'):
    """
...
def translate_tencent_back(text, from_='zh', to_='en'):
    ...
    """
    try:
        text_from_to = translate_tencent(text, from_=from_, to_=to_)
        text_to_from = translate_tencent(text_from_to, from_=to_, to_=from_)
        return text_to_from
    except Exception as e:
        logger.error(str(e))
        return None
